...
public class LoteriaWLAEngineManagerTest {
	...
	public void setUp() throws Exception {
		logger = Logger.getLogger(LoteriaWLAEngineManagerTest.class);
		gameParams = XMLUtil.parseResource("paymodel/LoteriaIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/LoteriaIW/InitOutcome.xml").getDocumentElement();
	...
	@Test
	public void test_buy_NoWin_RevealOneTime() throws Exception {
		Element playerInput;
		ODERevealDataOutcomeWrapper odeRevealDataOutcomeWrapper = new ODERevealDataOutcomeWrapper();

		// Wager
		playerInput = new PlayerInputBuilder().appendActionInput(buyAction).appendWagerInput(new Wager("Wager1", 1)).appendJSONInput("{'testData':0}")
				.build();
		...
		// Reveal 1
		playerInput = new PlayerInputBuilder().appendActionInput("play").appendJSONInput("{'revealStatus':0,'revealData':'test message'}").build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);

		assertJsonContent("{'revealData':'test message'}", JXPathUtil.getString(outcome, "//JSONOutcome[@name='ODERevealData']/text()"),
	...
	@Test
	public void test_buy_2x_100CreditsWin_RevealThreeTimes() throws Exception {
		Element playerInput;
		ODERevealDataOutcomeWrapper odeRevealDataOutcomeWrapper = new ODERevealDataOutcomeWrapper();

		// Wager
		playerInput = new PlayerInputBuilder().appendActionInput(buyAction).appendWagerInput(new Wager("Wager1", 2)).appendJSONInput("{'testData':0}")
				.build();
		Node outcome = manager.play(InputUtil.buildInput(gameParams, previousOutcome, playerInput));
		logger.debug(outcome);
		...
		playerInput = new PlayerInputBuilder().appendActionInput("play").appendJSONInput("{'revealStatus':1,'revealData':'msg1'}").build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);

		assertJsonContent("{'revealData':'msg1'}", JXPathUtil.getString(outcome, "//JSONOutcome[@name='ODERevealData']/text()"),
				odeRevealDataOutcomeWrapper);
		assertEquals("Reveal", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, 0, 2, 0);

		// Reveal 2 - without reveal data
		playerInput = new PlayerInputBuilder().appendActionInput("play").appendJSONInput("{'revealStatus':1,'revealData':null}").build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);
		...
		playerInput = new PlayerInputBuilder().appendActionInput("play").appendJSONInput("{'revealStatus':0,'revealData':'msg2'}").build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);

		assertJsonContent("{'revealData':'msg2'}", JXPathUtil.getString(outcome, "//JSONOutcome[@name='ODERevealData']/text()"),
				odeRevealDataOutcomeWrapper);
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
	...
	@Test
	public void test_try_NoWin() throws Exception {
		Element playerInput;

		// Wager
		playerInput = new PlayerInputBuilder().appendActionInput(tryAction).appendWagerInput(new Wager("Wager1", 1)).appendJSONInput("{'testData':0}")
				.build();
		TestODEProxy.setJsonResponse("{'prizeValue':0,'moreStuff':'toTest','prettyPrint':'forGhst'}");
		Node outcome = manager.play(InputUtil.buildInput(gameParams, previousOutcome, playerInput));
		logger.debug(outcome);
	...
	@Test
	public void test_try_2x_100CreditsWin() throws Exception {
		Element playerInput;

		// Wager
		playerInput = new PlayerInputBuilder().appendActionInput(tryAction).appendWagerInput(new Wager("Wager1", 2)).appendJSONInput("{'testData':0}")
				.build();
		TestODEProxy.setJsonResponse("{'prizeValue':100,'moreStuff':'toTest','prettyPrint':'forGhst'}");
		Node outcome = manager.play(InputUtil.buildInput(gameParams, previousOutcome, playerInput));
		logger.debug(outcome);
	...
	private void assertSettledPendingPayout(Node outcome, int settled, int pending, int payout) throws XPathExpressionException {
